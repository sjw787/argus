#!/usr/bin/env bash
# start.sh — Start AthenaBeaver frontend + backend
#
# Usage:
...
info()    { echo -e "${CYAN}[athena-beaver]${RESET} $*"; }
success() { echo -e "${GREEN}[athena-beaver]${RESET} $*"; }
warn()    { echo -e "${YELLOW}[athena-beaver]${RESET} $*"; }
error()   { echo -e "${RED}[athena-beaver]${RESET} $*" >&2; }

# ── Help ───────────────────────────────────────────────────────────────────────
if [[ "$MODE" == "--help" || "$MODE" == "-h" ]]; then
  echo -e "${BOLD}AthenaBeaver start script${RESET}"
  echo ""
...
# Activate venv so uvicorn/athena-beaver are on PATH
# shellcheck disable=SC1091
source "$VENV_DIR/bin/activate"

# Make sure athena_beaver package is importable
...
# ── PROD mode ──────────────────────────────────────────────────────────────────
if [[ "$MODE" == "prod" ]]; then
  STATIC_DIR="$SCRIPT_DIR/src/athena_beaver/api/static"
  ...
  echo ""
  echo -e "${BOLD}${GREEN}▶  AthenaBeaver — Production${RESET}"
  echo -e "   UI + API: ${CYAN}http://localhost:8000${RESET}"
  echo ""

  python -m uvicorn "athena_beaver.api.app:create_app" \
    --factory \
    --host 127.0.0.1 \
    --port 8000 &
  PIDS+=($!)
...
# ── DEV mode ───────────────────────────────────────────────────────────────────
  ...
  echo ""
  echo -e "${BOLD}${GREEN}▶  AthenaBeaver — Development${RESET}"
